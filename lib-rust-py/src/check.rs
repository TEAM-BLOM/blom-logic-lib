...
impl Check {
    ...
    pub fn check_win(&self) -> Result<bool, PyErr> /* build JsError, debug i32 */ {
        ...
        if turn == 0 {
            // return Err(0);
            return Err(PyErr::new::<PyValueError, _>("this position looks Invalid"));
        }
    ...
    pub fn check_33(&self) -> Result<bool, PyErr> /* build JsError, debug i32 */{
        let (_x, _y) = self.current;
    ...
    pub fn check_44(&self) -> Result<bool, PyErr> /* build JsError, debug i32 */ {
        let (_x, _y) = self.current;
        let dir: [(i32, i32); 4] = [(1, 0), (0, 1), (1, 1), (-1, 1)];
